...
- 🐚 **Fish → Zsh Konvertierung** *(v2.52.0)* — `sccs convert fish-to-zsh` generiert ein natives zsh-Profil aus der Fish-Konfiguration für Rechner **ohne fish** (z.B. Standard-zsh auf macOS): Aliase, Env-Vars **und Funktionen** werden Best-Effort übersetzt (inkl. `if`/`switch`-Blöcke), `*.macos.fish`/`*.linux.fish` landen in `uname`-Guards, und jede generierte Datei wird per `zsh -n` validiert — es wird nie kaputtes zsh erzeugt. Verteilung über die neue Kategorie `zsh_config`; Aktivierung einmalig via `source ~/.config/zsh/zshrc` → [docs/usage/platforms.md](docs/usage/platforms.md)
...
- 🐚 **Fish → Zsh conversion** *(v2.52.0)* — `sccs convert fish-to-zsh` generates a native zsh profile from your fish configuration for machines **without fish** (e.g. stock macOS zsh): aliases, env vars **and functions** are translated best-effort (incl. `if`/`switch` blocks), `*.macos.fish`/`*.linux.fish` are wrapped in `uname` guards, and every generated file is validated with `zsh -n` — broken zsh is never emitted. Distribute via the new `zsh_config` category; activate once with `source ~/.config/zsh/zshrc` → [docs/usage/platforms.md](docs/usage/platforms.md)
