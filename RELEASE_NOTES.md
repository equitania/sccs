...
- **Doctor integration is deliberately quiet.** Only the preset named by `statusline.active` produces a row, and only when it is missing (`statusline:<name>  MISSING  configured but not installed`); `sccs doctor install` then offers it as one confirm-gated action. Listing every available preset would be noise, and installing one the user never chose would be surprising. Like `cli_tools`, a missing statusline is informational and does not change the exit code.
...
- 31 new tests in `tests/test_statusline.py`, including that the installer reaches `bash` as a file argument, that no argv element ever contains a pipe, and the capture/restore round trip. Docs: [docs/usage/profiles.md](docs/usage/profiles.md#statusline) (DE/EN).
