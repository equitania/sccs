...
- **uv tools carry their source, Python and extras.** The inventory is captured with `uv tool list --show-python --show-extras --show-version-specifiers`: a tool the source installed from a local checkout or wheel (`file://…`) is recorded as `local` and reported on a mirror as *manual* — yellow, listed under the table, never an action that must fail; a `git+…` source installs with `uv tool install 'name @ git+url'`; a registry tool installs as `name[extras]==version --python X.Y`, so a tool that needs Python 3.12+ no longer fails against the mirror's default interpreter. Found on the first real mirror: four private tools from GitLab could not come from PyPI, and `eq-chatbot-core==3.3.0` was resolved against Python 3.10.
- **The Brewfile is dumped with `--formula --cask --tap`.** Homebrew 7 also dumps `uv "…"` and `vscode "…"` lines, and `brew bundle install` on a mirror then tried to fetch the private uv tools from PyPI. uv tools are the inventory's job; VS Code extensions are out of scope.
- **A fresh host's Fisher was reported as "not installed".** `fisher list` exits 1 while no plugin is recorded in the universal `_fisher_plugins` — exactly the state of a mirror whose `fisher.fish` arrived through the sync. The wrapper now checks `functions -q fisher` for presence and takes the list as it is, so every plugin in `fish_plugins` becomes *missing* and `doctor install` runs `fisher install <name>` for each.
...
- **Two rules, pinned by tests**: the source is never modified from the inventory, and a mirror never writes the inventory. Removals exist only under `optimize --strict` with `cleanup: true`; `npm`, `corepack` and `sccs` are never removed; Homebrew extras are computed against `brew leaves`, so a dependency is never offered for removal; a checkout with local changes is reported in yellow, without failing the check, and never touched. A non-strict `optimize` run on a mirror with extras and `cleanup: true` prints a single advisory block instead of queuing anything.
...
- **`fish_config`'s bundled include list now covers `fish_plugins`.** Without it, the Fisher area of mirror parity was inert on a fresh host: reconciliation reads `fish_plugins`, but nothing had ever synced the file there.
- **`doctor install`/`update`/`optimize` now fetch before comparing repos.** `_collect_doctor_statuses` only ever set `fetch=check_updates`, a flag only `doctor check` sets — so a mirror repo stuck `behind` never advanced past its last fetch, and the `git pull --ff-only` action for it never fired. A new `fetch_repos` parameter (defaulting to `check_updates`) lets install/update/optimize opt in explicitly; all three run `git fetch` per configured checkout, bounded to 30 seconds each.
- **A `modified` or unreadable (`not_a_repo`) checkout no longer fails `doctor check`.** Both are things only a human can fix, not gaps `doctor install` could ever act on — reported in yellow, the same as an extra, instead of counting as drift with exit 1 and print-only advice to "run doctor install" that would not have helped.
- **`sccs_inventory` is now a blocked deployment category** — like `claude_memories`/`claude_plans`/`claude_todos`, it may never ship in a `sccs deploy` profile: it names the source host and lists its uv tools and npm globals, which has no place on a customer server.
...
- **Two different facts about an installed artefact, both preserved**: `pre_existing` — something that already sat at the target path before SCCS ever wrote there — is neither written over by `install` nor removed by `revoke`. An artefact we installed that the customer later edited IS removed, and flagged `modified`, because it still carries our knowledge regardless of who touched it last.
- **A target SCCS did not write is skipped, not overwritten.** A deployment refreshes what it previously installed, but it never displaces a file that was already on the host: that file may hold somebody's hand edits. Such targets are listed as `skipped_foreign` by `deploy install`, in both the text and the `--json` output, so the operator sees what did not land and why. `pre_existing` itself is decided at the FIRST install and carried forward by every later one — the same ownership line `sccs integrations codex` draws with its `foreign_target` guard. The claim is keyed on the **resolved target path**, not on `(category, name)`: a category's `local_path` is not fixed for all time, and a later bundle resolving the same name to a different directory must still check what is actually there.
- **`~/.config/sccs/` is purged only when the deployment created it.** A host whose user runs `sccs` themselves keeps their `config.yaml`, `.sync_state.yaml` and `backups/`; `revoke` removes only the receipt and says so. The spec leaves `sccs` installed as a public tool, so its state directory is not automatically ours.
...
- **`revoke` ends with a verification sweep** that exits non-zero on leftovers: a removal that reports success while a directory survived is worse than no cleanup at all, because the report is what the decision to stop looking rests on. It has two passes. The first re-stats every planned removal; the second re-scans the receipt's sweep globs against the recorded category directories and **deliberately ignores the per-entry bookkeeping**, since that bookkeeping is precisely what a verification pass exists to second-guess. Only `retain` categories and shared artefacts are exempt outright.
- **Every finding is then classified three ways, and only one of the three is not a failure.** A name planned for removal that survived fails. A name in the bundle's sweep globs with *no* receipt entry fails — that is the signature of an install that wrote files without recording them. A name whose entry positively records that SCCS never wrote there, and whose content is not the artefact the bundle ships, is reported and clearly labelled but does **not** fail: `claude_framework` ships `CLAUDE.md` and Claude Code creates `~/.claude/CLAUDE.md` on nearly every host, so failing there would fail on essentially every customer machine, and an alarm that always fires is an alarm nobody reads. The `--json` payload carries the split as `leftovers` (failing) and `benign_leftovers`.
- **The exemption rests on a recorded fact and on content, never on an inference.** `pre_existing` says "the target existed, so we skipped" — it is written by the same code that makes the call, and a build that got the call wrong marked its own artefacts `pre_existing` while overwriting them. So `ReceiptEntry` now carries `written_by_sccs` as a separate provenance record, and for a skipped target also `shipped_hash`, the hash of what the bundle *would* have written there. A foreign target that turns out byte-identical to the shipped artefact fails the sweep: our knowledge is on that host whoever put it there. It is still never deleted — reported, not removed.
- **A failing finding keeps its profile's receipt record**, not only a failed removal. Losing it was the worst case in the feature: `revoke` exited 1 and printed the path, the operator re-ran, and with the record gone the second run printed "Nothing to revoke on this host" and exited 0 while the artefact was still there. The `unrecorded` case needs this most — without an entry there is nothing left to re-derive the directory from, so the sweep could never find that artefact again.
- **The receipt is version 2.** A version-1 file is refused rather than read under the new assumption, because it predates `written_by_sccs` and this build cannot tell which of its entries SCCS actually wrote. The error names both ways out: revoke from the SCCS version that wrote it (its `sccs_version` is in the file), or remove the artefacts by hand and delete the receipt.
- **`revoke --profile <unknown>` is an error**, not a clean host: it exits non-zero and names the profiles that ARE installed. A typo used to filter the plan to nothing and report success. An empty plan with no `--profile` filter stays the legitimate "nothing installed here" case.
...
### Fixed (a category whose `local_path` names a file — affects `sccs import` too, not only `deploy`)

- **`sccs import` and `sccs deploy install` crashed on any host that already had `~/.config/starship.toml`.** A single-file category (`starship_config`, `gitconfig`) carries the file itself as its `local_path`, and the importer appended the item name to it — building `~/.config/starship.toml/starship.toml` and raising `FileExistsError` in `mkdir()`, outside the per-item error handling, so the traceback escaped the whole import. In a deployment that left files on the customer host with no receipt written, after which `deploy status` reported "nothing installed" and `deploy revoke` "nothing to revoke". All four bundled profiles include `starship_config`, so this was the default path.
- The importer now mirrors the export side's convention (`sync.item.scan_items_for_category`) and resolves such a category to the file's parent directory. Directory-backed `file` categories — `claude_commands`, `claude_agents` and the rest — are untouched. Where the path already exists on the host, `resolves_to_parent()` additionally runs the export side's own `is_dir()` test, so a directory outvotes the manifest fingerprint — that closes the one case the manifest alone cannot see, a directory-backed category holding a single file named like the directory.
- **`deploy install` validates the receipt before writing anything.** A corrupt `.deploy_receipt.yaml` used to surface only in `record_install()`, i.e. after the files were on disk, producing the same orphaned-install shape as the crash above.
- `deploy export --platform macos` stamped the profile's own `target_platform` into the manifest instead of the override. The ZIP contents were already correct; only the manifest field lied, and the install host has no way to tell.
...
New coverage across `tests/test_deploy_*.py` for the profile schema (incl. the blocked-category and cycle-detection validators), profile resolution and platform targeting, bundle building with the generated cleanup command, the receipt, install with the home guard, and the revoke plan/sweep across all buckets — plus the two config regression tests above and a version-bump assertion in `tests/test_deploy_schema.py`.

Added in the pre-merge fix wave: sticky `pre_existing` across a reinstall and a foreign target that stays byte-identical (`tests/test_deploy_install.py`); a single-file category through `sccs deploy install`, plain `sccs import` and the no-config import path, with a directory-backed category as the control (`tests/test_transfer.py`); the unknown-`--profile` error in both output modes and both branches of the state-directory policy (`tests/test_deploy_cli.py`); a leftover the per-entry bookkeeping considers accounted for that the sweep catches anyway (`tests/test_deploy_revoke.py`); and the platform override reaching the manifest (`tests/test_deploy_bundle.py`).

Added in the corrective wave: the three-way sweep classification — a wrongly-skipped artefact of ours, a shipped name with no receipt entry at all, and a foreign target holding our exact content — each asserted to fail *and* to keep its profile's receipt record, against the case-3 counterpart that succeeds and lets it go (`tests/test_deploy_revoke.py`); the same scenario end to end through the CLI, where the operator's second `deploy revoke` must not report a clean host (`tests/test_deploy_cli.py`); provenance and the shipped hash recorded for both kinds of entry, and a relocated category still checking its new path (`tests/test_deploy_install.py`); the version-1 receipt refusal with its guidance (`tests/test_deploy_receipt.py`); and the on-disk single-file decision (`tests/test_transfer.py`). 1881 total; ruff/format/mypy clean.
...
- **`AGENTS.md` rewritten from scratch.** The previous file was a machine-translated copy of `CLAUDE.md` in which every occurrence of "Claude" had been replaced by "Codex" — including the subject matter itself, yielding claims like "SCCS merges `~/.Codex/settings.json`", a path that does not exist. Claude Code artefacts are what this tool synchronizes; Codex is one of the export targets. The new file is standalone guidance for an agent working *on* the repository and leads with exactly that warning.
- **The CAO agent profiles left this repository.** `agents/` described how work is routed across four agent CLIs — internal working knowledge that has no place in a package published to PyPI and mirrored to GitHub. It now lives in the private sync repository and is distributed through SCCS itself. `sccs capacity` stays; it is a feature of this package, and the CAO reference in `docs/usage/capacity.md` is only the reason the command exists.
...
- **`~` and `$HOME` in `statusLine.command` were never expanded, so a working statusline was reported as broken.** Claude Code runs that command through a shell, which is why both bundled presets use shell syntax — `~/.claude/statusline` and `"$HOME/.claude/statusline.sh"` — and why the `claude_statusline` sync category's own `settings_ensure` writes `~/.claude/statusline.sh`. The detector tested the raw `shlex.split` token, and `Path("~/.claude/statusline").is_file()` is always False. Result: `statusline: claude-statusline │ MISSING │ binary not found: ~/.claude/statusline` on every machine using a preset, next to a `statusline-preset:` row saying `installed · in use` about the very same file — the preset check expands via `resolve_marker()`, the integrity check did not. `doctor check` therefore exited 1 permanently and `doctor install` could only print a manual block that nothing could ever clear. Both the binary and the script token now go through `_expand_command_token()` (`expanduser` + `expandvars`); the report keeps the raw value and appends where it actually looked (`~/.claude/statusline (→ /home/you/.claude/statusline)`). An unexpandable `$NOPE` still fails the existence check, which is the honest answer.
- **`doctor check --json` was missing the statusline preset rows.** v2.58.1 added `statusline-preset:` to the Rich table but not to the JSON payload, so a GUI consuming `--json` (sccs-gui) saw only the `status_lines` integrity check and could not tell which preset is chosen, installed or live — it got the false alarm above and nothing to explain it. The payload now carries `statusline_presets`. It lists **every** known preset with its `installed` / `is_active` / `is_configured` / `version` flags rather than the table's filtered view — a machine payload should hand over the facts and let the consumer decide what to render.
- **The chosen preset is recorded in config.yaml.** `sccs statusline use|install` wrote only `~/.claude/settings.json`, which is machine state: it does not survive a rebuild, and it is not what `doctor install` reads. Both commands now persist `statusline.active` via the new `save_statusline_active()` — best-effort, so an unwritable config warns instead of failing a switch that already happened, and a no-op when the value is unchanged, because re-serializing YAML drops the user's comments.
- **`doctor install` offers the installer for a preset that is configured *or* live**, the same condition the report row uses. `sccs profile off <name>` points `settings.json` at the profile's `statusline_fallback_preset` without touching config.yaml, so gating the action on `statusline.active` alone made the doctor print a `MISSING` row for a statusline it then refused to install — exactly the case that shows up after parking GSD on a fresh machine. The MISSING row now also says *which* of the two applies (`configured but not installed` vs. `in use but not installed`) instead of claiming "configured" in both cases.

16 new tests (`TestStatusLineTokenExpansion` in `tests/test_doctor.py`, install-action and CLI round-trip tests in `tests/test_statusline.py`, `TestSaveStatuslineActive` in `tests/test_config.py`, the JSON payload in `tests/test_cli_json.py`). Full suite: 1481 passed.

## Version 2.58.1 (14.08.2026)

### Fixed (doctor report)

- **The doctor reports the statusline in every state, not only when it breaks.** The first cut printed a row only for a *missing* preset, on the theory that a healthy one is noise. That was wrong, and reported as such: `doctor check` is a status table where `npx:`, `skill:`, `browsers:` and `perm:` all print an OK row, so silence on a working statusline was indistinguishable from SCCS not knowing about it at all. A preset that is configured as `active` **or** currently live now always gets a row — `installed · in use`, `installed · not in use — \`sccs statusline use <name>\``, or `MISSING — \`sccs statusline install <name>\`` — with the binary's own version in the Version column via the new `version_arg` field. `installed · not in use` is the case worth naming: the preset is chosen and present, but an extension like GSD still owns `statusLine`. Presets that are neither configured nor live stay out; the table is a status, not a catalogue (`sccs statusline list` shows all of them, now with version and install hint too). Installing remains one confirm-gated action in `doctor install`, and a missing statusline stays informational — it does not change the exit code.
- **Version detection does not go through `doctor.runner._run`.** That validator requires argv[0] to look like a PATH name and rejects an absolute path — which is exactly what a statusline marker is, so the Version column silently stayed empty. `detect_version()` spawns the binary directly instead, keeping the safety properties locally: `shell=False`, a real argv list, a `version_arg` validated to be a single flag, stdin closed, a 5-second timeout, and every failure degrading to `None`. Output longer than 40 characters is discarded rather than smeared across the table.
- Docs corrected in the same pass: `docs/usage/profiles.md` (DE/EN) documented the old "only reports when missing" behaviour and now shows all three row states plus the new `version_arg` field.

12 new tests (43 in `tests/test_statusline.py`), covering each row state, the omission rule, and version detection degrading on a non-executable marker or a binary that answers with prose. Full suite: 1465 passed.

## Version 2.58.0 (14.08.2026)

### Added (statusline presets)

- **`sccs statusline list|use|install|show` — choose which statusline Claude Code runs.** Direct follow-up to 2.57.0: GSD owns `settings.json.statusLine` (`hooks/gsd-statusline.js`), so parking the profile left the entry pointing at a script that is no longer wired up and the status line went blank. 2.57.0 fell back to a hardcoded file name; that is now a named preset, and the choice is configuration rather than a constant.
- **A preset names one statusline**: `command`, optional `padding`, `marker_path` (whose existence proves it is installed), `install_url` / `install_url_windows`, and `managed_paths`. Bundled: `builtin` (`"$HOME/.claude/statusline.sh"` — the plain shell script many setups already carry) and `claude-code-statusline` ([glauberlima/claude-code-statusline](https://github.com/glauberlima/claude-code-statusline) — directory, git branch, file changes, model, context bar, cost). Configured under the new top-level `statusline:` key; `ProfileSpec.statusline_fallback` became `statusline_fallback_preset` and the bundled `gsd` profile now falls back to `claude-code-statusline`.
- **Installing never pipes curl into a shell.** `install_preset()` fetches the installer to a temp file with a plain argv list, checks it is neither empty nor larger than 512 KB, and hands it to `bash` as a *file argument*. Nothing built from configuration is ever interpreted by a shell. On top of that, `install_url` is validated at config-load time — https only, and the host must be on a three-entry allowlist (`raw.githubusercontent.com`, `github.com`, `objects.githubusercontent.com`) — so a hostile value surfaces during `sccs config validate` rather than at the moment someone confirms an install. The action is confirm-gated and defaults to **No**, and the prompt names the URL being executed. Windows aborts with the PowerShell command printed instead.
- **The binary must not reach the repository.** `claude-code-statusline` installs a multi-megabyte binary into `~/.claude/` next to a machine-local `statusline.toml` — squarely inside sync scope. Every preset's `managed_paths` now flow into `get_doctor_managed_excludes()`, which grew an optional `statusline_config` parameter threaded through `SyncEngine`, `Exporter` and `Importer`. Contributed by **all** known presets rather than the active one, so switching statuslines does not un-exclude the binary the previous one left behind. Your own `statusline.sh` is not a managed path and keeps syncing.
- **A typo in `statusline_fallback_preset` aborts the profile switch** instead of silently leaving the statusline pointing at the parked extension — a blank status line with no explanation is the worse failure.
- **`--no-use` now keeps its promise.** Found by running the real install: `claude-code-statusline`'s `install.sh` patches `settings.json.statusLine` itself as its last step, so `sccs statusline install --no-use` left the statusline changed anyway — on a machine with GSD switched on, that silently replaced the active statusline. The command now captures the `statusLine` block before installing and restores it afterwards when `--no-use` is given, reporting `the installer changed statusLine — reverted it`. New `StatusLineManager.current_block()` / `restore_block()`.
- 43 new tests in `tests/test_statusline.py`, including that the installer reaches `bash` as a file argument, that no argv element ever contains a pipe, and the capture/restore round trip. Docs: [docs/usage/profiles.md](docs/usage/profiles.md#statusline) (DE/EN).

### Fixed (config loader)

- **Optional top-level config blocks were dropped unless someone remembered to list them.** `_merge_with_defaults` carried one hand-written `if "<block>" in data` branch per optional block; a block without a branch never reached the Pydantic model — no error, just settings with no effect. This had already bitten twice (`doctor:` overrides, and `pi.base_dir`/`pi.exclude` until v2.53.0), and it bit again immediately: the new `statusline:` block validated fine and still came back as `active=None`. It would have hit the `profiles:` block from 2.57.0 the same way, so user-defined profiles never worked either.
- The per-block branches are replaced by one loop that derives the pass-through set from `SccsConfig.model_fields`, minus the six keys that genuinely need a deep merge (`_DEEP_MERGED_KEYS`). Unknown keys are still ignored — the pass-through is driven by the model, not by "anything goes". A parametrised regression test in `tests/test_config.py` asserts every optional block survives the merge and fails if this ever reverts to an allowlist.

Full suite: 1453 passed.

## Version 2.57.0 (14.08.2026)

### Added (profiles)

- **`sccs profile on|off|list|status` — switch a whole extension's artefacts off without deleting them.** An extension installed by `sccs doctor` writes skills, agents, `settings.json` hooks and a statusline into `~/.claude/` as one unit, but there was no way to take that unit out of the way for a day's work. On a typical setup `@opengsd/gsd-core` contributes 71 skills, 34 agents and 19 hook entries — every description is loaded into the model's system prompt at session start, and the eight `PreToolUse` guards among those hooks spawn a process on *every* Read, Write and Bash call, whether or not the extension is used. Measured on the maintainer's setup, the skill and agent frontmatter alone accounts for ≈3,700 tokens before a single word is typed.
- **Nothing is deleted — everything moves.** `deactivate` relocates matching skill directories and agent files to `~/.config/sccs/profiles/<name>/`, removes matching hook entries from `settings.json` (recording event, matcher and the entry itself), and — only when the current `statusLine` command matches one of the profile's own patterns — swaps it for `statusline_fallback` while storing the previous value. `activate` reverses each step. A name collision in the parking area raises `ProfileError` instead of overwriting: that state means the same artefact exists in both places, and a blind move would be data loss. Hook *files* under `~/.claude/hooks/` stay where they are — they cost no context and do not run without an entry in `settings.json`.
- **Restore is a three-way merge, and it preserves the original grouping.** The first implementation re-inserted each removed hook into the first entry with a matching `matcher` — which looked right until it met a real `settings.json`. A live setup routinely holds *several* outer entries sharing one matcher under the same event; merging by matcher collapsed them (`PreToolUse` 8 entries → 7, `SessionStart` 4 → 2). Functionally equivalent, but SCCS exists to move configuration around without rewriting it, so `deactivate` now also records the full pre-strip entry list of every event it touches (`ProfileRecord.original_hook_events`) and `activate` rebuilds from that shape. The merge resolves all three cases: a hook we removed comes back in its original entry; a hook added while parked is kept and appended; a hook the user *deleted* while parked stays deleted. Verified against the maintainer's real 19-entry `settings.json` — a full off/on cycle returns a semantically identical file. `settings.json` is parsed and rewritten before any file is moved, so a malformed file aborts with `~/.claude/` untouched.
- **The doctor had to learn about it, or the next pass would undo everything.** `sccs doctor install/update` re-runs `npx @opengsd/gsd-core --force-statusline`, which writes the artefacts straight back. New `DoctorConfig.installable_npx_tools()` drops the npx tools of a switched-off profile — mirroring the existing `effective_plugins()` / `checkable_plugins()` split. That single filter also removes the tool from the bundled-skill, browser-bundle and GSD-orphan detectors, so a parked profile produces no MISSING rows.
- **`effective_npx_tools()` deliberately stays profile-blind.** It feeds `get_doctor_managed_excludes()`, which is what keeps `gsd-*` out of `sccs sync`. Had the filter been applied there instead, parking GSD would have made sync start picking up any `gsd-*` leftovers in `~/.claude/` and pushing them to the repository. A regression test pins this. The parking area itself lives outside `~/.claude/` and is therefore out of sync scope regardless.
- **`doctor.protected_hooks` (default `gsd-`) is unchanged** and still wins over `disallowed_hooks`. A profile switch is an explicit, user-initiated operation and overrides that guard by design — the guard exists against *automatic* stripping, not against a deliberate `sccs profile off`.
- **Profiles are configurable, not hardcoded.** The bundled `gsd` spec lives in `sccs/doctor/profiles.py:DEFAULT_PROFILES`; the new top-level `profiles:` key in `config.yaml` adds new ones or fully replaces a bundled spec. Fields: `skills` / `agents` (fnmatch globs against names in `~/.claude/skills/` and `~/.claude/agents/`), `hooks` (substring patterns, same semantics as `doctor.disallowed_hooks`), `statusline_fallback` (bare file name — a path or `..` is rejected at validation) and `npx_tools`. Profile names are restricted to lowercase letters, digits, `-` and `_`.
- **Known limitation, documented rather than worked around**: a switch takes effect in the *next* Claude Code session, because skills, agents and hooks are read once at session start. `sccs profile on|off` and `sccs profile list` both say so in their output.
- 33 new tests in `tests/test_profiles.py` cover the round trip, hook-grouping preservation, a user deletion made while parked, idempotence in both directions, the collision guard, the malformed-`settings.json` abort, the unrelated-statusline case, and the sync-exclude regression. Docs: [docs/usage/profiles.md](docs/usage/profiles.md) (DE/EN).
