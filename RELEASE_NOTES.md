...
- The importer now mirrors the export side's convention (`sync.item.scan_items_for_category`) and resolves such a category to the file's parent directory. Directory-backed `file` categories — `claude_commands`, `claude_agents` and the rest — are untouched.
